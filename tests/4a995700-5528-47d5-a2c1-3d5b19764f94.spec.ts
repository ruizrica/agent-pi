...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Vuln Scanner Installer', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/vuln-scanner-installer/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — Rate limit exceeded', async ({ request }) => {
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('rate limit exceeded')
    })
  })
