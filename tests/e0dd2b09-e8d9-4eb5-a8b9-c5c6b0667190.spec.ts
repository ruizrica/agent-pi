...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Claude Cli', () => {
    // Module with 6 exported items Background:
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/claude-cli/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — , (err) => {', async ({ request }) => {
