...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Cleanup Viewer Html', () => {
    // Module with 3 exported items Background:
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/cleanup-viewer-html/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — >Error:', async ({ request }) => {

      // Act — API Call
      // When: the request triggers >Error:
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('>error: ')
    })

    test('Handle error case — </span>', async ({ request }) => {

      // Act — API Call
      // When: the request triggers </span>
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      const body = await response.json()
      const errorBody = body
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('</span>')
    })
  })
