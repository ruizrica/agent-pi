...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Security Engine', () => {
    // Module with 28 exported items Background:
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/security-engine/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — [security-engine] Failed to parse ${path}: ${err}', async ({ request }) => {
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('[security-engine] failed to parse ${path}: ${err}')
    })
  })
