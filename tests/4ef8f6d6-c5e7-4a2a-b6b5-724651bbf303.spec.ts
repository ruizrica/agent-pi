  ...
  test.describe('Droid Cli', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/droid-cli/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — Object', async ({ request }) => {

      // Act — API Call
      // When: the request triggers object
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('object')
    })
  })
