...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Commander Prompt', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/commander-prompt/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — Status', async ({ request }) => {

      // Act — API Call
      // When: the request triggers status
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('status')
    })

    test('Handle error case — <progress>', async ({ request }) => {

      // Act — API Call
      // When: the request triggers <progress>
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      const body = await response.json()
      const errorBody = body
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('<progress>')
    })
  })
