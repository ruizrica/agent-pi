  ...
  test.describe('Learn', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/learn/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — Success', async ({ request }) => {

      // Act — API Call
      // When: the request triggers success
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('success')
    })
  })
