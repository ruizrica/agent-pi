...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Qa Rico Html', () => {
    // Module with 5 exported items Background:
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/qa-rico-html/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — <div class=', async ({ request }) => {

      // Act — API Call
      // When: the request triggers <div class=
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      const body = await response.json()
      const errorBody = body
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('<div class=')
    })
  })
