...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Completion Report Html', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/completion-report-html/${id}`, {
        })
      }
    })
    ...
    test('Handle error case —  + idx +', async ({ request }) => {
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain(' + idx + ')
    })

    test('Handle error case — Unknown error', async ({ request }) => {

      // Act — API Call
      // When: the request triggers unknown error
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      const body = await response.json()
      const errorBody = body
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('unknown error')
    })
  })
