...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Pr Review Report Html', () => {
    // Module with 4 exported items Background:
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/pr-review-report-html/${id}`, {
        })
      }
    })
    ...
    test('Handle error case —  : undefined);', async ({ request }) => {

      // Act — API Call
      // When: the request triggers  : undefined);
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
