...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Chrome Devtools Mcp', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/chrome-devtools-mcp/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — Request failed', async ({ request }) => {

      // Act — API Call
      // When: the request triggers Request failed
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('request failed')
    })
  })
