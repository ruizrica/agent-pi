  ...
  test.describe('Pipeline Render', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/pipeline-render/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — , theme.bold(', async ({ request }) => {
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain(', theme.bold(')
    })
  })
