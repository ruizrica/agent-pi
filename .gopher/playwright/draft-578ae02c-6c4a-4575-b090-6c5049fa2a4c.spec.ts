...
import { test, expect } from '@playwright/test'
  ...
  test.describe('Memory Cycle Helpers', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/memory-cycle-helpers/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — ) continue;', async ({ request }) => {
    ...
    test('Handle error case — Assistant', async ({ request }) => {

      // Act — API Call
      // When: the request triggers assistant
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('assistant')
    })
  })
