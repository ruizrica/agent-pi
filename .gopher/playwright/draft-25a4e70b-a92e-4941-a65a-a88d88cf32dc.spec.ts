  ...
  test.describe('Tasks', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/tasks/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — New list:', async ({ request }) => {
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('new list: ')
    })
  })
