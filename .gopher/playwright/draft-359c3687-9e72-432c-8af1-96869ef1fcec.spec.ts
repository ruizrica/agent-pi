  ...
  test.describe('Viewer Server', () => {
    ...
    test.afterAll(async ({ request }) => {
      for (const id of createdIds) {
        await request.delete(`http://localhost:8787/api/viewer-server/${id}`, {
        })
      }
    })
    ...
    test('Handle error case — Invalid JSON', async ({ request }) => {

      // Act — API Call
      // When: the request triggers Invalid JSON
      // TODO: Implement API call for this action
      const response = await request.get(`${process.env.API_BASE_URL}/TODO`)
      ...
      expect(JSON.stringify(errorBody).toLowerCase()).toContain('invalid json')
    })
  })
