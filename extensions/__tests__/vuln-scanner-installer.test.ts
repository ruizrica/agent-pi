// ABOUTME: Test suite for the AI project vulnerability scanner protection installer — validates file generation and installation logic.
...
function createTestDir(files: Record<string, string> = {}): string {
	counter++;
	const dir = join(tmpdir(), `vuln-scanner-install-test-${Date.now()}-${counter}`);
