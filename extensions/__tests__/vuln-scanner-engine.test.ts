// ABOUTME: Test suite for the AI project vulnerability scanner engine — validates vulnerability detection, project profiling, and report generation.
...
function createTestProject(files: Record<string, string>): string {
	testCounter++;
	const dir = join(tmpdir(), `vuln-scanner-test-${Date.now()}-${testCounter}`);
