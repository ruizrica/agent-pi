// ABOUTME: /vuln-scanner command extension — AI Project Vulnerability Scanner and Protection Installer.
// ABOUTME: Scans projects for AI security vulnerabilities (prompt injection, credential exposure) and installs portable security guards.
/**
 * /vuln-scanner — AI Project Vulnerability Scanner & Protection Installer
 *
 * Subcommands:
 *   /vuln-scanner           — Run full vulnerability sweep (scans project for AI vulnerabilities)
 *   /vuln-scanner sweep     — Same as above
 *   /vuln-scanner install   — Install AI protection files into current project
 *   /vuln-scanner status    — Show current project's security posture (quick check)
 *   /vuln-scanner report    — View last vulnerability report
 *
 * The sweep detects:
...
export default function vulnScanner(pi: ExtensionAPI) {
	...
	pi.registerCommand("vuln-scanner", {
		description: "AI Project Vulnerability Scanner — scan for vulnerabilities, install protections [sweep|install|status|report]",
		handler: async (args, ctx) => {
			...
			switch (subcommand) {
				...
				case "help":
					ctx.ui.notify(
						[
							"🔍 /vuln-scanner — AI Project Vulnerability Scanner & Protection",
							"",
							"Commands:",
							"  /vuln-scanner              Run full vulnerability sweep",
							"  /vuln-scanner sweep        Same as above",
							"  /vuln-scanner install      Install AI protections into project",
							"  /vuln-scanner install --overwrite  Overwrite existing files",
							"  /vuln-scanner status       Quick security posture check",
							"  /vuln-scanner report       View last vulnerability report",
							"  /vuln-scanner help         Show this help",
						].join("\n"),
						"info",
					);
					break;
...
async function handleSweep(cwd: string, ctx: any, pi: ExtensionAPI) {
	ctx.ui.notify("🔍 Running AI vulnerability sweep...", "info");
	...
	try {
		...
		const reportPath = join(reportDir, "vulnerability-sweep-report.md");
		...
		pi.sendMessage(
			{
				customType: "vulnerability-sweep-result",
				content: report,
				display: true,
			},
		...
		ctx.ui.notify(`Vulnerability sweep failed: ${err}`, "error");
	}
}
...
async function handleInstall(cwd: string, ctx: any, args: string, pi: ExtensionAPI) {
	...
	try {
		...
		const reportPath = join(reportDir, "vulnerability-install-report.md");
		writeFileSync(reportPath, report, "utf-8");
		...
		pi.sendMessage(
			{
				customType: "vulnerability-install-result",
				content: report,
				display: true,
			},
			{ deliverAs: "followUp", triggerTurn: true },
...
async function handleStatus(cwd: string, ctx: any) {
	try {
		...
		checks.push({
			...
			pass: hasGuard,
			detail: hasGuard ? "Installed" : "Not installed — run /vuln-scanner install",
		});
		...
		checks.push({
			...
			pass: hasPolicy,
			detail: hasPolicy ? "Present" : "Not found — run /vuln-scanner install",
		});
		...
		const lines = [
			`🔍 Vulnerability Status — ${profile.name}`,
			``,
...
async function handleReport(ctx: any, pi: ExtensionAPI) {
	...
	const cwd = ctx?.cwd || process.cwd();
	const reportPath = join(cwd, ".pi", "vulnerability-sweep-report.md");
	...
	if (existsSync(reportPath)) {
		...
		ctx.ui.notify("No vulnerability report found. Run /vuln-scanner sweep first.", "info");
	}
}
