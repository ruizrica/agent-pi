...
export function statusButton(
  ...
  const inv = theme.inverse
    ? (t: string) => theme.inverse!(t)
    : (t: string) => t;
  ...
  for (let i = 0; i < phases.length; i++) {
    ...
    if (phase.status === "done") {
      ...
      if (phase.summary) {
        ...
        const summary = maxSummary > 0
          ? truncate(phase.summary, maxSummary)
          : "";
      ...
      const prefixVisible = INDENT.length + visibleWidth(statusBtn);
      const separatorGap = "  ";
      const separatorLen = Math.max(
        0,
        width - prefixVisible - separatorGap.length,
      );
      ...
      for (const agent of phase.agents) {
        ...
        const timeStr =
          (agent.status === "running" || agent.status === "done" ||
              agent.status === "error") && agent.elapsed > 0
        ...
        const padLen = Math.max(
          0,
          width - prefixLen - gap.length - workLen - timeSectionLen,
        );
        const agentLine = prefixStr + gap + theme.fg("muted", work) +
          " ".repeat(padLen) + (timeLen > 0 ? theme.fg("dim", timeStr) : "");
