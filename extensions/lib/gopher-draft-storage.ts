...
function generateUUID(): string {
  // Simple UUID v4 fallback - crypto.randomUUID is available in Node.js 14.17+
...
function ensureDraftDirs(
  cwd: string,
): { gherkinDir: string; playwrightDir: string } {
...
export function storeDrafts(
  ...
  for (const feature of features) {
    ...
    writeFileSync(
      gherkinMetaPath,
      JSON.stringify(gherkinMeta, null, 2),
      "utf-8",
    );
    ...
    if (hasPlaywright) {
      ...
      const playwrightMetaPath = join(
        playwrightDir,
        `draft-${playwrightId}.meta.json`,
      );
      ...
      writeFileSync(
        playwrightMetaPath,
        JSON.stringify(playwrightMeta, null, 2),
        "utf-8",
      );
    }
